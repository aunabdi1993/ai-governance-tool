...
This module provides the OpenAIProvider class which implements the AIProvider
interface for OpenAI's GPT models (GPT-4, GPT-3.5, etc.). It includes comprehensive
error handling, retry logic, and token usage tracking.

Example:
    >>> from ai_governance.providers import OpenAIProvider
...
class OpenAIProvider(AIProvider):
    ...
    Note: Prices vary by model and region. Check OpenAI's pricing page for current rates.
    ...
    Attributes:
        ...
        input_price_per_1k: Input price per 1K tokens (model-specific).
        output_price_per_1k: Output price per 1K tokens (model-specific).
    ...
    def __init__(
        ...
        Args:
            ...
            model: Model to use for refactoring (default: gpt-4-turbo).
            max_tokens: Maximum tokens for API responses (default: 4096).
        ...
        # Set pricing for the model
        model_info = self.MODEL_PRICING.get(self.model, self.MODEL_PRICING.get("gpt-4-turbo"))
        self.input_price_per_1k = model_info["input"]
        self.output_price_per_1k = model_info["output"]
    ...
    def refactor_code(
        ...
        try:
            ...
            logger.debug(f"Calling OpenAI API for refactoring {filepath}")
    ...
    def refactor_with_context(
        ...
        try:
            ...
            logger.debug(f"Calling OpenAI API with context for {filepath}")
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert code refactoring assistant for multi-file codebases.",
                    },
                    {"role": "user", "content": prompt},
                ],
            )
    ...
    def _build_refactor_prompt(
        ...
        """Build the refactoring prompt for OpenAI.
        ...
        if isinstance(error, APIError):
            ...
            # Quota errors (should not retry)
                ...
                exc = ProviderQuotaError(
                    ...
                    "Please check your usage at https://platform.openai.com/account/billing/overview "
